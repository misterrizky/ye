<div class="card mb-5 mb-xl-10">
    <div class="card-header border-0 pt-5">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label fw-bolder fs-3 mb-1">Bank</span>
        ...
        <div class="card-toolbar" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-trigger="hover" title="{{__('custom.click_return')}}">
    ...
    <div class="card-body py-3">
        <form id="form_input">
            <div class="row">
                <div class="col-lg-6">
                    <label class="required fs-6 fw-bold mb-2">{{__('custom.name')}}</label>
                    <input type="text" class="form-control" name="name" placeholder="{{__('custom.enter')}} {{__('custom.name')}}..." value="{{$data->name}}">
                </div>
                <div class="min-w-150px mt-10 text-end">
                    @if ($data->id)
                    <button id="tombol_simpan" onclick="handle_save('#tombol_simpan','#form_input','{{route('office.bank.update',$data->id)}}','PATCH');" class="btn btn-sm btn-primary">{{__('custom.update')}}</button>
                    @else
                    <button id="tombol_simpan" onclick="handle_save('#tombol_simpan','#form_input','{{route('office.bank.store')}}','POST');" class="btn btn-sm btn-primary">{{__('custom.save')}}</button>
                    @endif
                </div>
            </div>
        </form>
